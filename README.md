...
3.14 開発テンプレート。DevContainer サポート付き。

## 特徴

### 開発環境

- **Python 3.14** - 最新の安定版 Python
- **DevContainer** - Docker 内の完全な VS Code 開発環境
- **PostgreSQL 17** - 開発用データベース（オプション）
- **SQLAlchemy 2.0 + psycopg 3** - モダンな ORM とドライバー構成
...
17 が含まれています（PostgreSQL を有効にしている場合）。
...
- **接続文字列**：`postgresql+psycopg://${POSTGRES_USER}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB}`
